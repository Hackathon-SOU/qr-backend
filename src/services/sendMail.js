...
const verifyLink =
  process.env.ENV === "development"
    ? process.env.VERIFYLINKDEV
    : process.env.VERIFYLINKPROD;
...
const sendVerificationMail = async (name, adminMail, password) => {
  try {
    ...
    await ejs
      .renderFile(path.join(path.resolve(), "src/templates/verifyEmail.ejs"), {
        userName: name,
        password: password,
...
const sendSuccessfulVerifiedMail = async (name, adminMail) => {
  try {
    ...
    mailOptions.to = adminMail;
    await ejs
      .renderFile(path.join(path.resolve(), "src/templates/welcomeEmail.ejs"), {
        userName: name,
      })
      .then((result) => {
        mailOptions.html = result;
